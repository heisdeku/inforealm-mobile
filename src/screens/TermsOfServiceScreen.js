...
const TermsOfServiceScreen = () => {
    ...
    const getTerms = async () => {
        try {
            setError('');
            setIsLoading(true)
            const response = await apiConnect.get('/getTermsOfService');
            if(response.data.status === 'success'){
                const termsObject = response.data.terms;
                setTitle(termsObject.title);
                setDate(termsObject.last_updated);
                setContent(termsObject.content);
                setIsLoading(false);
            }
        } catch (error) {   
            console.log(error);
